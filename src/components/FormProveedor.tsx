...
import { RegistrarProveedor } from '../api/adminapi'
...
export default function FormProveedor() {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            ...
            const response = await RegistrarProveedor(proveedorData)

            if (response.data.success) {
                toast.success(response.data.message || "Proveedor registrado correctamente")
                // Limpiar formulario
                reset()
                setTipoPersona("")
                setRegionSeleccionada("")
            } else {
                toast.error(response.data.message || "Error al registrar el proveedor")
            }
        } catch (error: any) {
            console.error("Error al registrar proveedor:", error)
            const errorMessage = error.response?.data?.message || error.response?.data?.error || "Error al registrar el proveedor"
                ...
                <CardContent>
                    ...
                    <div className="flex items-start gap-3 p-4 mb-6 bg-yellow-50 border border-yellow-200 rounded-lg">
                        ...
                        <p className="text-sm text-yellow-800">
                            ...
                            Una vez registrado, el proveedor podrá ser desactivado o editado.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                id="nombre"
                                placeholder={tipoPersona === "juridica" ? "Ej: Distribuidora ABC Ltda." : "Ej: Juan Pérez González"}
                                {...register("nombre", {
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                id="direccion"
                                placeholder="Ej: Calle Los Aromos 456"
                                {...register("direccion", {
